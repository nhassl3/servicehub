...
<header>
	<h2 class="h-2"><a href="#right-section-buy" class="link cur-d">Корзина</a>
		<div class="above-header" id="counts-of-goods">{{ $countCartItems }}</div>
	</h2>
</header>
	...
	<article class="left-zone">
		...
		<article class="goods">
			<section class="left-section-buy">
				...
				@php $firstElementId = array_key_first($products) @endphp
				@foreach($products as $key => $product)
				@if($key !== $firstElementId)
				<hr class="w50-line" style='margin-bottom: 1.7em;'>
				@endif
				<div class="product-card" data-cart_item_id="{{$key}}">
					<section class="card">
						<label class="custom-checkbox" for="checkbox-{{$key}}">
							<input type="checkbox" data-checkbox_id="{{$key}}" checkbox-id="checkbox-element" id='checkbox-{{ $key }}'>
							<span class="custom-checkbox__checkmark"></span>
							<span class="custom-checkbox__label"></span>
						</label>
						<img src="{{ asset($product['image_url']) }}" alt="shopping-cart-good-{{ $key }}">
					</section>
					<div class="desc">
						...
						<div class="panel">
							<div class="manage-panel">
								<button class="like {{ $product['isLike'] ? "clicked" : "" }} manage-panel-button a32-panel" data-cart_item_id='{{ $key }}'>
								...
								<button class="manage-panel-button a32-panel pd-wm32" data-cart_item_id={{ $key }} onclick="updateUrlParams({ cartItemId: +this.dataset.cart_item_id }, '/gocheckout');">
									<div class="fast-buy">
					...
					<div class="price-block">
						<div class="price-with-discount">
							<span class="price">{{ number_format($product['discount_price'], 0, ".", " ") }} ₽ </span><br><span class="with-discount">
								со скидкой</span>
						</div>
						...
						<div class="change-count">
							...
							<div inputmode="numeric" pattern="[1-9]*" min="1" max="10" class="numeric-input">
								<div class="set-numeric">
									<input inputmode="numeric" pattern="[1-9]*" type="number" class="count-of-good" min="1" max="10" value="{{ $product['quantity'] }}" data-cart_item_id="{{ $key }}" id='count-{{ $key }}'>
								</div>
							</div>
